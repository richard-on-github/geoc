...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      >
        {!isCollapsed && (
          <div className="flex min-w-0 items-center gap-2.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[hsl(var(--sidebar-primary))]">
              <img src={logoLonato} alt="Lonato" className="h-full w-full object-contain" />
            </div>
            <div className="min-w-0">
              <p className="text-sm leading-tight font-semibold text-white">GEOC</p>
              <p className="truncate text-[10px] opacity-50">LONATO Togo</p>
            </div>
          </div>
        )}
        {isCollapsed && (
          <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-[hsl(var(--sidebar-primary))]">
            <img src={logoLonato} alt="Lonato" className="h-full w-full object-contain" />
          </div>
        )}
        {!isCollapsed && (
          <button
            ...
            className="ml-2 rounded-md p-1.5 text-[hsl(var(--sidebar-foreground))] opacity-50 transition-colors hover:bg-[hsl(var(--sidebar-accent))] hover:opacity-100"
          >
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-2" aria-label="Menu principal">
        {isCollapsed && (
          <div className="flex justify-center py-1">
            <button
              ...
              className="rounded-md p-1.5 text-[hsl(var(--sidebar-foreground))] opacity-50 transition-colors hover:bg-[hsl(var(--sidebar-accent))] hover:opacity-100"
            >
      ...
      <div className="space-y-0.5 border-t border-[hsl(var(--sidebar-border))] p-2">
        {visibleBottom.map((item) => (
          <SidebarNavItem key={item.id} item={item} isCollapsed={isCollapsed} />
        ))}
        {!isCollapsed && (
          <p className="px-3 py-1 text-center text-[10px] text-[hsl(var(--sidebar-foreground))] opacity-30">
            v{env.appVersion}
          </p>
        )}
      </div>
