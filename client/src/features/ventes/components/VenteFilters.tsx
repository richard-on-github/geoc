...
import toast from 'react-hot-toast'
import { useAgences } from '@/features/agences/hooks'
import { useImportVentes, useClotures } from '../hooks'
import { ventesApi } from '../api'
import { useDebounce } from '@/shared/hooks/useDebounce'
import { Can } from '@/shared/components/navigation/Can'
import { cn } from '@/shared/lib'
...
interface VenteFiltersProps {
  onFilterChange: (filters: VenteFiltersState) => void
  /** Vue active (Jours/Mois/Années/Général) : détermine quels contrôles afficher. */
  viewMode: VenteViewMode
  /**
   * Filtre jour/mois/année dérivé de la vue active (voir useVenteBrowserState).
   * Fusionné dans les paramètres d'export, pour que l'export porte toujours
   * sur exactement ce qui est affiché à l'écran (+ les filtres additionnels
   * ci-dessous, s'ils sont renseignés).
   */
  periodeFilters: VentePeriodeFilters
}

const PERIODE_REGEX = /^\d{4}-(0[1-9]|1[0-2])$/

export function VenteFilters({ onFilterChange, viewMode, periodeFilters }: VenteFiltersProps) {
  const [searchInput, setSearchInput] = useState('')
  const [agenceId, setAgenceId] = useState('')
  const [dateDebut, setDateDebut] = useState('')
  const [dateFin, setDateFin] = useState('')
  ...
  // Période du mois pour lequel le fichier est importé (désormais exigée par le backend :
  // toute ligne du fichier n'appartenant pas à ce mois fait rejeter l'import en entier).
  const [periodeImport, setPeriodeImport] = useState('')
  ...
  // "Non clôturées uniquement" n'a de sens que sur la vue Générale : dans les
  // vues Jours/Mois/Années, un mois peut être partiellement clôturé et ce
  // filtre entrerait en conflit avec le contexte jour/mois/année déjà actif.
  // On le masque (et on ne l'applique plus) en dehors du Général plutôt que de
  // laisser un filtre invisible rester actif silencieusement. Le filtre
  // clotureId (choisir une clôture précise), lui, reste disponible partout.
  const estVueGenerale = viewMode === 'general'
  ...
  const periodeImportValide = PERIODE_REGEX.test(periodeImport)

  const handleConfirmImport = () => {
    if (!selectedFile || !periodeImportValide) return
    importVentes(
      { file: selectedFile, periode: periodeImport },
      {
        onSuccess: () => {
          setSelectedFile(null)
          setPeriodeImport('')
  ...
  const handleExport = async (format: 'csv' | 'excel' | 'pdf') => {
    try {
      ...
      const params = {
        ...
        // Applique immédiatement le filtre de la vue active (jour/mois/année) :
        // si on regarde le jour 34, l'export ne porte que sur ce jour-là, etc.
        // Les filtres ci-dessus (recherche, agence, dates, clôture) s'y ajoutent.
        ...periodeFilters,
      }
      const password = await ventesApi.exportVentes(params, format)
  ...
  return (
    <div className="mb-4 space-y-4">
      {/* 1. Ligne Principale : Barre de recherche et filtres (inchangés, valables sur toutes les vues) */}
      <div className="flex flex-wrap items-center gap-4">
        <div className="relative max-w-sm min-w-50 flex-1">
          <Search className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" />
          <input
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--muted))]/30 p-2">
        {estVueGenerale ? (
          ...
          // Vues Jours/Mois/Années : la clôture est gérée de façon contextuelle
          // (bouton Clôturer/Annuler affiché uniquement sur la vue "Mois", voir
          // VentesBrowser), donc pas de case à cocher ici. On garde l'espace
          // pour ne pas faire sauter la mise en page des boutons d'action.
          <span />
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-lg">
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-[hsl(var(--foreground))]">
                  Période du fichier (mois auquel appartiennent ces ventes)
                </label>
                <input
                  type="month"
                  value={periodeImport}
                  onChange={(e) => {
                    setPeriodeImport(e.target.value)
                  }}
                  className="w-full rounded-(--radius) border border-[hsl(var(--input))] p-2 focus:ring-2 focus:ring-[hsl(var(--ring))] focus:outline-none"
                />
                <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">
                  Toute ligne du fichier dont la date ne correspond pas à ce mois entraînera le
                  rejet complet de l'import.
                </p>
              </div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                type="button"
                onClick={handleConfirmImport}
                disabled={isImporting || !periodeImportValide}
                className="rounded-md bg-[hsl(var(--primary))] px-4 py-2 text-sm font-medium text-[hsl(var(--primary-foreground))] hover:opacity-90 disabled:opacity-50"
              >
                {isImporting ? 'Importation en cours...' : "Confirmer l'import"}
              </button>
