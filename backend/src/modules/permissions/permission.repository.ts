...
import type {
  PermissionAllQueryParams,
  PermissionQueryParams,
} from "./permission.interface.js";

type PermissionUserRelation = {
  user: {
    id: string;
    [key: string]: unknown;
  };
...
const formatPermission = (permission: any) => ({
  ...permission,
  users: permission.users.map((up: PermissionUserRelation) => up.user),
  roles: permission.roles.map((rp: PermissionRoleRelation) => rp.role),
});
